...
import { updateCancelOrderAPI } from "../../../helper/controller/order";
export default function WaitGetProductOrder({ navigation, route }) {
  ...
  const handleCancelOrder = async (account, saleOrderId) => {
    condition.saleOrderId = null;
    condition.isReviewed = 1;
    condition.isOutput = 0;
    condition.isDelivery = 0;
          ...
          renderItem={({ item, index }) => {
            return (
              ...
              >
                ...
                >
                  <TouchableOpacity
                    onPress={() => handleCancelOrder(account, item.saleOrderId)}
                    style={{
                      borderRadius: 5,
